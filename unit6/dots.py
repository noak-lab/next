...
def connect_the_dots(dots: tuple) -> None:
    """
    connects all the dots in given array and shows result on the screen
    :param dots: a list of all the results
    :return: None
    :raise ValueError: raises an exception if there are less than two dots
    """
    screen = pygame.display.set_mode((300, 300))
    screen.fill((234, 212, 252))
    if len(dots) < 4:
        raise ValueError
    for i in range(0, len(dots) - 3, 2):
        pygame.draw.line(screen, (0, 0, 255), (dots[i], dots[i+1]), (dots[i+2], dots[i+3]), 2)
    pygame.display.flip()
    running = True
    while running:
